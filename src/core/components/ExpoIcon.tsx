...
export interface IIconProps extends StackProps {
  color: keyof ThemeParsed;
  size: number;
}
...
const Icon = (props: any) => {
  ...
  return (
    <Stack flexGrow={0} flexShrink={0} {...rest}>
      {props.children}
    </Stack>
  );
};

export const StartIcon = ({ size, color, ...rest }: IIconProps) => {
  const theme = useTheme();
...
export const HourglassIcon = ({ size, color, ...rest }: IIconProps) => {
  const theme = useTheme();
  const iconColor = theme[color]?.val as string;
  return (
    <Icon {...rest}>
...
export const PlusIcon = ({ size, color, ...rest }: IIconProps) => {
  const theme = useTheme();
  const iconColor = theme[color]?.val as string;
  return (
    <Icon {...rest}>
